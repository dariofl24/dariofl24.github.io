...
(function(app, $) {
    ...
    PickUpPointMap.prototype = {
        ...
        populate: function() { console.log("%%%%%% POPULATE");
            var that = this;
            var geoCodedCount = 0;

            if(this.pickUpPointListByDist.length<=0){ console.log("this.pickUpPointListByDist.length<=0");
                ...
                $.each(this.pickUpPointList.getData(), function(id, pickUpPoint) {
                    if (('latitude' in pickUpPoint) && ('longitude' in pickUpPoint)){
                        ...
                        geoCodedCount++;
                        console.log("&&&& "+pickUpPoint.latitude+","+pickUpPoint.longitude);
                    }
                });
                ...
                $.each(this.pickUpPointListByDist, function(id, pickUpPoint) {
                    ...
                    if( count< that.cache.limitFR){
                        if (('latitude' in pickUpPoint.pup) && ('longitude' in pickUpPoint.pup)){
                            that.createMarker(pickUpPoint.pup);
                            geoCodedCount++;
                            console.log("&&&& "+pickUpPoint.pup.latitude+","+pickUpPoint.pup.longitude);
                        }
                    }else{
                        return false;
                    }
                    count=( that.increasePopulateCounter() )?(count+=1):0;
        ...
        increasePopulateCounter: function(){
            return app.util.siteInfo().countryCode===this.cache.countryCodeFR;
        },
        ...
        getPUPDistance: function(pickUpPoint){
            ...
            console.log("("+pickUpPoint.latitude+","+pickUpPoint.longitude+") - ("+that.cache.myCoordinates.lat+","+that.cache.myCoordinates.lng+")");
            var distance=0;
            ...
            console.log("PUP:: "+pickUpPoint);
            console.log("-Distance: "+pickUpPoint.distance);
            deferred.resolve();
        ...
        sortAllPUPByDistance: function(){
            ...
            for (var pickUpPointID in this.pickUpPointList.data) {
                console.log(i+"-PUP-IDD: "+pickUpPointID);
        ...
        sortPUPsBydistance: function(){
            ...
            for (var pickUpPointID in this.pickUpPointList.data) {
                console.log("-isGeocoded: "+that.pickUpPointList.data[pickUpPointID].geocoded);
                
                if( !(that.pickUpPointList.isGeocoded() && (app.util.siteInfo().countryCode!==that.cache.countryCodeFR)) ){
                    console.log("GCP-YES");
                    if(that.pickUpPointList.data[pickUpPointID].geocoded){
                        console.log("PUSH-isGeocoded: "+that.pickUpPointList.data[pickUpPointID].geocoded);
                        pairs.push({
            ...
            $.each(pairs, function(id, obj) {
                var address = obj.pup.address;
                console.log( [("D:: "+obj.distance), address.street, address.city, address.postalCode].join(' '));
            });
        ...
        geocodePickUpPoint: function(pickUpPoint) {
            ...
            console.log("FORMATED-ADDRESS: "+address+" - "+app.util.siteInfo().countryCode+" ; "+postalCode);
        ...
        init: function() {
            ...
            console.log("$$isGeocoded: "+this.pickUpPointList.isGeocoded());
            
            if (this.pickUpPointList.isGeocoded() && (app.util.siteInfo().countryCode!==this.cache.countryCodeFR) ) {
